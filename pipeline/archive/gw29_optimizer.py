"""
GW29 Single-Gameweek Optimizer
================================
Completely standalone from Stage 8 / Intel 06.

Objective: pick the single best 15-man squad + starting XI to maximise
predicted points in GW29 only.  No future horizon.  No transfer costs.
No chip logic.  Just the highest possible predicted GW29 score.

Pipeline:
  1. Load player_history.csv (2025-26 GW1-29) + supporting FPL data
  2. Build per-player, per-GW rolling features from GW1-28 actuals
  3. Train 4 fresh XGBoost models (GK/DEF/MID/FWD) on those features
  4. Snapshot GW29 features for every player (using GW1-28 history)
  5. Apply intel adjustments: FDR adj, zero-minutes filter, ownership boost
  6. ILP optimizer -> best 15-man squad + XI
  7. Print squad report including actual GW29 points alongside predictions
