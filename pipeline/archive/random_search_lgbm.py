"""
pipeline/random_search_lgbm.py
Random hyperparameter search using LightGBM instead of XGBoost.
Also includes FDR_MULT_DEF (position-specific FDR for GK/DEF) in search space.
...
N_TRIALS     = 250
RESULTS_DIR  = os.path.join(_ROOT, "data", "intel", "random_search_lgbm")
SUMMARY_PATH = os.path.join(RESULTS_DIR, "summary.json")

# Best known XGBoost config as starting point for trial 1 (baseline comparison)
BEST_KNOWN = {
    "FDR_MULT":           0.025,
    "FDR_MULT_DEF":       0.025,
...
def main():
    ...
    print("=" * 60)
    print(f"  LGBM RANDOM SEARCH — {n_trials} trials")
    print(f"  Search space: {total_vals} values across {len(SEARCH_SPACE)} params")
    print(f"  Includes FDR_MULT_DEF (position-specific GK/DEF penalty)")
    print(f"  Estimated runtime: ~{n_trials * 80 // 60} minutes")
    print("=" * 60)
    ...
    for trial_id in range(start_id, n_trials + 1):
        ...
        if trial_id == 1:
            print(f"\n  Trial 1: best known config with LGBM (baseline)")
        ...
        valid = [r for r in results if r.get("status") == "ok"]
        valid.sort(key=lambda x: x["total"], reverse=True)
        remaining_s = (n_trials - trial_id) * 80
    ...
    print("\n" + "=" * 60)
    print("  LGBM RANDOM SEARCH COMPLETE — TOP 10 RESULTS")
    print("=" * 60)
    ...
    if baseline:
        print(f"\n  LGBM baseline (trial 1): {baseline['total']} pts")
        print(f"  XGBoost best:            1716 pts")
        delta = valid[0]['total'] - 1716
        sign  = "+" if delta >= 0 else ""
        print(f"  vs XGBoost best:         {sign}{delta} pts")
