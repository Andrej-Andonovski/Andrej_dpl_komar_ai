...
- Stage 9 ✅ LLM agent (Codex API) — per-GW narrative explanations
...
Per-GW explanations via Codex API (post-simulation analysis):
  - Reads season_simulation.json GW-by-GW squad decisions
  - Calls Codex once per GW to explain why each player was picked
  - Output: models/stage9_explanations.json (narrative per GW)
  - MODEL_ID: Codex-sonnet-4-20250514
...
models/stage9_explanations.json — Codex narrative per GW
