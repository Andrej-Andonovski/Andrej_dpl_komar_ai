...
Crucially, industry analysis establishes that reaching the \textbf{top 1,000 overall} — the top 0.01\% of all participants — requires sustaining an average of approximately \textbf{64 points per gameweek} across the full season, equivalent to roughly 2,450 total points over 38 gameweeks. The system achieved \textbf{64.3 points per gameweek} over GW1--28, placing it precisely at — and marginally above — the threshold required for a \textbf{global top 1,000 finish} out of over 10 million managers. This positions the system within the \textbf{top 0.01\%} of the entire FPL player base.

This result is achieved without any manual intervention: every transfer, captaincy, and chip decision is made autonomously using only publicly available data (FPL API, Understat, Fantasy Football Scout). An additional indicator of strategic quality is the complete absence of transfer penalty points across all 28 gameweeks — suboptimal transfer management is a common failure mode for both human managers and naive automated approaches, yet was entirely avoided through the combination of free-transfer banking logic, chip-lockout rules, and loyalty bonuses in the ILP optimiser.
